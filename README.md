...
| 自助工具 | 可配置番茄钟（自定义时长 + 休息倒数 + 关联任务与追踪项）、呼吸锚定、启动困难急救、复盘三问、规律性诊断 |

### 番茄钟不是固定的 25 分钟

- **时长全可调**：专注 / 短休 / 长休 / 几轮后长休，四个值都能改，改完立刻生效
- **休息也倒计时**：专注结束后自动进入休息并倒数，每 N 个番茄自动切长休
- **能挂到具体的事上**：关联当日某个任务，计时时显示在表盘下方，完成后可选自动勾掉它
- **完成即自动记录**：关联一个追踪项（比如「阅读」），专注结束自动给它加上设定的数量，不用手动补记
- **手动跳过按实际时长入账**：中途结束不会白干，已专注多少分钟就记多少
- **切后台不会偷走你的时间**：倒计时按绝对时间戳计算，而不是每秒减一。浏览器会把后台标签的定时器限流到约每分钟一次，递减式计时会把 25 分钟实际拖成 40 分钟
- **刷新不丢**：计时状态写进本地存储，关掉页面再打开会接着走
